...
``HNSWIndex/searchKnn(_:maxResults:ef:labelFilter:)`` invokes your closure with each candidate’s **label id** (the same `id` passed to ``HNSWIndex/addPoint(_:id:metadata:)``). Return `true` to allow the label in the result set.
...
``HNSWIndex/searchKnn(_:maxResults:ef:filter:)`` passes the stored metadata string, or `nil` if no metadata exists. You decide whether `nil` matches.
...
Highly selective filters (few passing labels) may return fewer than `k` neighbors unless the search explores enough candidates. Increase the per-query `ef` argument so the underlying search can discover enough matches when they exist.
